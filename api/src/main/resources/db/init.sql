...
insert into addresses values
  ...
  (3, 'Kozielska', 1),
  (4, 'Pulawska', 4),
  (5, 'Czeslawa Przybylskiego', 4),
  (6, 'Zlota', 1),
  (7, 'Zlota', 4),
  (8, 'Strzelcow bytomskich', 1);
...
insert into addresses_people(person_id, address_id) values
  (1, 1),
  (1, 4),
  (1, 5),
  (2, 5),
...
select setval('addresses_people_id_seq', (select max(id) from addresses));

insert into cities(name) values ('Bytom'), ('Katowice'), ('Zabrze');
insert into people(name, age) values ('Jan', 46), ('Beata', 44);
...
class AnormPostRepository extends PostRepository {

  def all: Seq[Post] = DB.withConnection { implicit c =>
    AnormJsonRepository.selectByJson.foreach(println)
